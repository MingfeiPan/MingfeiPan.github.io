...
这里大致就是PEP-492的内容, 总结了很多其实用起来并没有这么复杂, 剩余还有关于async with跟async for内容请大家自行阅读PEP。

还有些有趣的内容, 比如asyncio之前已经写好了, 所以必然有一些兼容性的改动以配合async/await:  
...
关于yield from到async/await算是一路了解下来, 总感觉yield from更像是走了一段"弯路", 如果中间不绕这么一段我想python协程会更好学习一些。
