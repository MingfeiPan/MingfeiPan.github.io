...
如果只是一个几百的质数, 那么很快就会被穷举完毕, 但是实际中我们可能会使用1024位或者2048位的质数, 那么穷举的代价会是2^1024, 2^2048, 这是非常非常巨大的操作了, 几乎不可能, 这也就是该算法立足的原因    

当然Diffie-Hellman key exchange并非完美无缺, 他也很有显著的弱点, 比如无法防御Man-in-the-middle attack(中间人攻击), 所以在其基础上又产生了Oakley密钥算法, 这个就留到之后再说吧...   
